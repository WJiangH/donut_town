...
export class ThemeService {
  constructor({store,catalog,memberFor,keyFor,adminKeys='',onChanged=()=>{}}) {
    Object.assign(this,{store,catalog,memberFor,keyFor,adminKeys,onChanged});
  }
  current() {
  ...
  async view(userId) {
    ...
    return {current,themes:this.catalog,canManage:canManageTheme(member,userId ? this.keyFor(userId) : '',this.adminKeys),canSave:this.store.configured};
  }
  async change(userId,body) {
    if (!userId) throw new Error('slack_login_required');
    const member=await this.memberFor(userId);
    if (!canManageTheme(member,this.keyFor(userId),this.adminKeys)) throw new Error('town_admin_required');
