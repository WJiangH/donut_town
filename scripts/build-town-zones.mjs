...
const entries = [
  ...
  ...banks.map(spot => ({ action: "lookout", x: spot.x, y: spot.y, radius: 1.9, seats: 2 })),
...
${entries.map(entry => `  { action: "${entry.action}", scene: "town", x: ${entry.x}, y: ${entry.y}, radius: ${entry.radius}, seats: ${entry.seats} }`).join(",\n")}
];
`);
